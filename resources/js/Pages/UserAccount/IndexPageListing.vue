<template>
    <div class="max-w-7xl md:w-full mx-auto">
        <h2 class="text-2xl font-heading font-semibold leading-7 text-gray-800 mb-5">Your listings</h2>
...
const mobileDevices = ref(props.userListings);
...
const storageOptions = computed(() => {
    const uniqueStorage = new Set(props.userListings.map(device => device.storage_capacity));
    return Array.from(uniqueStorage).sort((a, b) => a - b);
});

const conditionOptions = computed(() => {
    const uniqueConditions = new Set(props.userListings.map(device => device.condition));
    return Array.from(uniqueConditions).sort();
});

const colorOptions = computed(() => {
    const uniqueColors = new Set(props.userListings.map(device => device.color));
...
const healthOrRamHeader = computed(() => {
    return props.userListings.some(device => device.phone_brand?.name?.toLowerCase() === 'apple') ? 'Health' : 'RAM';
});
</script>
